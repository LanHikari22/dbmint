...
# fmt: off
from typing import Optional, Tuple, List, TypeVar, Union
# fmt: on
import unittest
...
class SqliteDumpParser:
    ...
    @staticmethod
    def parse_tables_and_columns_from_dump(
        dump: str,
    ) -> Result[List[TableColumnsData], Error]:
        parsing_table = False
        ...
        for line in lines:
            ...
            if not parsing_table:
                ...
                table_syntax_end = "("
                if line.startswith(table_syntax_start) and line.endswith(
                    table_syntax_end
                ):
                ...
                if line == ");":
                    ...
                    if (
                        len(mut_varnames) != 0
                        and len(mut_vartypes) != 0
                        and len(mut_varsettings) != 0
                        and mut_table_name != ""
                    ):
                    ...
                    if len(words) < 2:
                        ...
                        if words[0] in "FOREIGN":
                            ...
                            from_key = (
                                words[1].replace("KEY(", "").replace(")", "").strip()
                            )
                            to_key = words[3].replace(",", "").strip()
                            to_key_table = to_key.split("(")[0]
                            to_key_col = to_key.split("(")[1].replace(")", "")

                            mut_foreign_key_defns.append(
                                (from_key, (to_key_table, to_key_col))
                            )
    ...
    def parse_inserts(s: str) -> Result[List[InsertData], Error]:
        ...
        for line in lines:
            ...
            if line.startswith(prefix):
                ...
                values_syntax_end = ");"
                if values_syntax_begin not in line or not line.endswith(
                    values_syntax_end
                ):
                    return Err(
                        Error(
                ...
                values = (
                    ...
                    | pipe.Of[str].to(lambda s: list(s.split(",")))
                    | pipe.OfIter[str].map(lambda s: s.strip())
...
class TableCsvDir:
    ...
    def export_csv(
        dir_name: str,
        table_columns: SqliteDumpParser.TableColumnsData,
        inserts: List[SqliteDumpParser.InsertData],
    ) -> IO[Result[None, Error]]:
        ...
        with open(f"{MOUNT_PATH}{dir_name}/{table_name}.csv", "w") as fw:
            ...
            inserts = (
                inserts
                | pipe.OfIter[SqliteDumpParser.InsertData].filter(
                    lambda ins: ins.table_name == table_name
                )
                    ...
                    return IO(
                        Err(
                            Error(
                                ...
                                data={"table_columns": table_columns, "insert": insert},
                            )
                        )
                    )
    ...
    def export_directory(
        dir_name: str,
        tables_columns: List[SqliteDumpParser.TableColumnsData],
        inserts: List[SqliteDumpParser.InsertData],
    ) -> IO[None]:
        os.system(f"mkdir -p {MOUNT_PATH}{dir_name}")

        for table_columns in tables_columns:
            res = TableCsvDir.export_csv(dir_name, table_columns, inserts).extract(
    ...
    def import_directory(dir_name: str) -> IO[List[SqliteDumpParser.InsertData]]:
        csv_filenames = (
            ...
            | pipe.OfIter[str].filter(lambda s: s.endswith(".csv"))
            | pipe.OfIter[str].to_list()
        )
        ...
        for csv_filename in csv_filenames:
            (
                TableCsvDir.import_csv(
                    f"{MOUNT_PATH}{dir_name}/{csv_filename}"
                ).extract(caller_returns_IO=True)
                | pipe.Of[List[SqliteDumpParser.InsertData]].to(
                    lambda xs: results.extend(xs)
                )
            )
...
class SyncCommandsImpl:
    ...
    class Rename:
        ...
        def from_str(
            ...
            sync_rename_linenos = (
                dbml_schema_lines
                | pipe.OfIter[str].enumerate()
                | pipe.OfIter[Tuple[int, str]].filter(
                    pipe.tup2_unpack(
                        lambda _, line: line.strip().startswith(rename_prefix)
                    )
                )
                | pipe.OfIter[Tuple[int, str]].map(pipe.tup2_unpack(lambda i, _: i))
            ...
            sync_rename_linenos_lines = (
                ...
                | pipe.OfIter[Tuple[int, str]].filter(
                    pipe.tup2_unpack(
                        lambda i, _: i in sync_rename_linenos
                        or (i - 1) in sync_rename_linenos
                    )
                )
                | pipe.OfIter[Tuple[int, str]].to_list()
            )
            ...
            orig_table_defn_lines = (
                dbml_schema_lines
                | pipe.OfIter[str].map(lambda line: line.strip())
                | pipe.OfIter[str].enumerate()
                | pipe.OfIter[Tuple[int, str]].filter(
                    pipe.tup2_unpack(
                        lambda _, line: line.startswith("Table ")
                        or line.startswith(table_defn_prefix)
                    )
                )
                | pipe.OfIter[Tuple[int, str]].map(
                    pipe.tup2_unpack(
                        lambda i, line:
                        ...
                        (
                            ...
                            if i > 0
                            and dbml_schema_lines[i - 1]
                            .strip()
                            .startswith(table_defn_prefix)
                            else (i, line)
                        )
                    )
                )
            ...
            def get_corresponding_table_defn(
                lineno: int, line: str
            ) -> Result[str, str]:
                table_defns_prior_or_equal_to_line = (
                    orig_table_defn_lines
                    | pipe.OfIter[Tuple[int, str]].filter(
                        pipe.tup2_unpack(lambda i, _: lineno >= i)
                    )
                    | pipe.OfIter[Tuple[int, str]].to_list()
                )
                ...
                defn = table_defns_prior_or_equal_to_line[-1][1] | pipe.Of[str].to(
                    lambda defn_line: defn_line.replace(table_defn_prefix, "")
                    .replace("Table", "")
                    .replace("{", "")
                    .strip()
                )
            ...
            enumerated_lines_with_defn_res = (
                ...
                | pipe.OfIter[Tuple[int, str]].map(
                    pipe.tup2_unpack(
                        lambda i, line: get_corresponding_table_defn(i, line).and_then(
                            lambda defn: Ok((i, line, defn))
                        )
                    )
                )
                | pipe.OfResultIter[Tuple[int, str, str], str].flatten()
            )
            ...
            def parse_before_line(
                line: str, table_ident: str
            ) -> Result[Tuple[bool, str, str, str], str]:
                if not line.strip().startswith(rename_prefix):
                    return Err(
                        f"Invalid before line. Must have a sync rename command: {line}"
                    )

                is_table = "Table" in line

                ident = (
                    line.replace(rename_prefix, "")
                ...
                if is_table and ident != table_ident:
                    return Err(
                        f"Table ident {ident} and {table_ident} must be identical. Line: {line}"
                    )

                return Ok((is_table, ident, table_ident, line))

            def parse_after_line(
                line: str, table_ident: str
            ) -> Result[Tuple[bool, str, str, str], str]:
                if line.strip().startswith(rename_prefix):
                    return Err(
                        f'Invalid after line. Must not have a sync rename command. Line: "{line}"'
                    )

                is_table = "Table" in line

                ident = line.replace("Table", "").replace("{", "").strip()

                return Ok((is_table, ident, table_ident, line))

            parsed_items_res = (
                enumerated_lines_with_defn
                | pipe.OfIter[Tuple[int, str, str]].enumerate()
                | pipe.OfIter[Tuple[int, Tuple[int, str, str]]].map(
                    pipe.tup2_right_tup3_flatten
                )
                | pipe.OfIter[Tuple[int, int, str, str]].map(
                    pipe.tup4_unpack(
                        lambda i, _lineno, line, defn: (
                            parse_before_line(line, defn)
                            if i % 2 == 0
                            else parse_after_line(line, defn)
                        )
                    )
                )
                | pipe.OfResultIter[Tuple[bool, str, str, str], str].flatten()
            )
            ...
            parsed_items_grouped2 = (
                ...
                | pipe.OfIter[int].map(lambda i: (parsed_items[i], parsed_items[i + 1]))
                | pipe.OfIter[
                    Tuple[Tuple[bool, str, str, str], Tuple[bool, str, str, str]]
                ].to_list()
            )

            def construct_self(
                tup_before: Tuple[bool, str, str, str],
                tup_after: Tuple[bool, str, str, str],
            ) -> SyncCommandsImpl.Rename:
                is_table1, ident1, table_ident1, line1 = tup_before
                is_table2, ident2, table_ident2, line2 = tup_after

                return SyncCommandsImpl.Rename(
                    (
                        SyncCommandsImpl.RenameEntities.RENAME_TABLE
                        if is_table1
                        else SyncCommandsImpl.RenameEntities.RENAME_COLUMN
                    ),
                    table_ident1,
                    ident1,
                    ident2,
            ...
            results = (
                ...
                | pipe.OfIter[
                    ...
                    pipe.tup2_unpack(
                        lambda tup_before, tup_after: construct_self(
                            tup_before, tup_after
                        )
                    )
                )
                | pipe.OfIter[SyncCommandsImpl.Rename].to_list()
            )
        ...
        def process_rename(
            ...
            List[SqliteDumpParser.TableColumnsData], List[SqliteDumpParser.InsertData]
        ]:

            new_foreign_key_defns_per_table = (
                table_columns
                | pipe.OfIter[SqliteDumpParser.TableColumnsData].map(
                    lambda data: data.foreign_key_defns
                    | pipe.OfIter[Tuple[str, Tuple[str, str]]].map(
                        pipe.tup2_right_tup2_flatten
                    )
                    ...
                    | pipe.OfIter[Tuple[str, str, str]].map(
                        pipe.tup3_unpack(
                            lambda from_id, to_table, to_col: (
                                (from_id, (self.after, to_col))
                                if (
                                    self.entity_type
                                    == SyncCommandsImpl.RenameEntities.RENAME_TABLE
                                    and to_table == self.table
                                )
                                else (
                                    ...
                                    if (
                                        ...
                                        and to_table == self.table
                                        and to_col == self.before.split(" ")[0]
                                    )
                                    else (from_id, (to_table, to_col))
                                )
                            )
                        )
                    )
                    | pipe.OfIter[Tuple[str, Tuple[str, str]]].to_list()
                )
                | pipe.OfIter[List[Tuple[str, Tuple[str, str]]]].to_list()
            )

            if self.entity_type == SyncCommandsImpl.RenameEntities.RENAME_COLUMN:
                new_table_columns = (
                    table_columns
                    | pipe.OfIter[SqliteDumpParser.TableColumnsData].enumerate()
                    | pipe.OfIter[Tuple[int, SqliteDumpParser.TableColumnsData]].map(
                        pipe.tup2_unpack(
                            lambda i, data: (
                                SqliteDumpParser.TableColumnsData(
                                    data.name,
                                    ...
                                    | pipe.OfIter[str].map(
                                        lambda varname: (
                                            self.after.split(" ")[0]
                                            if varname == self.before.split(" ")[0]
                                            else varname
                                        )
                                    )
                                    | pipe.OfIter[str].to_list(),
                                ...
                                if data.name == self.table
                                else SqliteDumpParser.TableColumnsData(
                                    data.name,
                                    data.varnames,
                                    data.vartypes,
                                    data.varsettings,
                    ...
                    | pipe.OfIter[SqliteDumpParser.TableColumnsData].to_list()
                )
                ...
                new_inserts = (
                    inserts
                    | pipe.OfIter[SqliteDumpParser.InsertData].map(
                        lambda insert: (
                            SqliteDumpParser.InsertData(self.after, insert.values)
                            if insert.table_name == self.table
                            else insert
                        )
                    )
                    | pipe.OfIter[SqliteDumpParser.InsertData].to_list()
...
SyncCommands = Union[SyncCommandsImpl.Rename]


def verify_path_no_folders_and_has_correct_ext(
    path: str, valid_ext: str, must_exist: bool = True
) -> Result[str, str]:
    ...
    return (
        ...
        if ext != valid_ext
            ...
            Err(
                "Invalid directory. Please specify a file in the specified mountpoint with no folders"
            )
            if dir != ""
            else (
...
def get_and_verify_db_filename(
    args: argparse.Namespace, key: str, must_exist: bool = False
) -> Result[str, str]:
    """
    Ensures that `key` is a path that ends with *.db and contains no directories
    """
    return verify_path_no_folders_and_has_correct_ext(
        getattr(args, key), ".db", must_exist
    )


def get_and_verify_dbml_schema_filename(
    args: argparse.Namespace, key: str, must_exist: bool = True
) -> Result[str, str]:
    return verify_path_no_folders_and_has_correct_ext(
        getattr(args, key), ".dbml", must_exist
    )


def unwrap_or_print_and_exit_on_app_err(
    result: Result[T, Error], exit_code: int = 1
) -> IO[T]:
    if result.is_ok():
...
def verify_schema_and_db_and_preproc_schema(
    args: argparse.Namespace, schema_arg: str, db_arg: str, writing_to_db: bool = True
) -> IO[Tuple[str, str, str, str]]:
    """
    ...
    # fmt: off
    db_filename = (
        dbml_schema_basename
        + 
            ".db" if getattr(args, db_arg) == None else 
            get_and_verify_db_filename(args, db_arg, must_exist=not writing_to_db).unwrap()
    )
    # fmt: on

    dbml_schema_proc_filename = f"{MOUNT_PATH}{dbml_schema_basename}.proc.dbml"

    # TODO t000.0: Deprecate this in favor of formal dbmt language
    # First, we apply the cpp preprocessor. This allows for file includes as well as type rename defines.
    os_system(
        f"cpp {MOUNT_PATH}{dbml_schema_filename} -P -o {dbml_schema_proc_filename}"
    )
...
def produce_db_from_data_dir(datadir: str, output_db_filename: str) -> IO[None]:
    ...
    _, base_inserts = gen_and_parse_sqlite3_dump_of_db(output_db_filename).extract(
        caller_returns_IO=True
    )

    inserts = TableCsvDir.import_directory(datadir).extract(caller_returns_IO=True)

    # Let's make sure to remove from inserts any identical inserts in base_inserts
    def insert_in_base(
        base_inserts: List[SqliteDumpParser.InsertData],
    ...
    inserts = (
        inserts
        | pipe.OfIter[SqliteDumpParser.InsertData].filter(
            lambda ins: not insert_in_base(base_inserts, ins)
        )
        | pipe.OfIter[SqliteDumpParser.InsertData].to_list()
    )
...
def gen_and_parse_sqlite3_dump_of_db(
    db_filename: str,
) -> IO[
    ...
    table_columns = SqliteDumpParser.parse_tables_and_columns_from_dump(
        dump_str
    ).unwrap()
...
def recreate_sqlite3_dump_of_db(
    table_columns: List[SqliteDumpParser.TableColumnsData],
    inserts: List[SqliteDumpParser.InsertData],
) -> str:
    mut_result = ""
    ...
    for table in table_columns:
        ...
        for varname, vartype, varsettings in zip(
            table.varnames, table.vartypes, table.varsettings
        ):
            mut_result += f"  {varname} {vartype} {varsettings} \n"

        for i, (from_key, (to_key_table, to_key_col)) in enumerate(
            table.foreign_key_defns
        ):
            mut_result += (
                f"  FOREIGN KEY({from_key}) REFERENCES {to_key_table}({to_key_col})"
            )
...
def _app_export_data(db_filename: str, datadir: str) -> IO[None]:
    table_columns, inserts = gen_and_parse_sqlite3_dump_of_db(db_filename).extract(
        caller_returns_IO=True
    )
...
def app_gen(args: argparse.Namespace) -> IO[None]:
    ...
    # TODO t000.1 Using *.proc.dbml which should be deprecated with *.dbmt
    os_system(
        f"dbml_sqlite {MOUNT_PATH}{dbml_schema_basename}.proc.dbml -f > {MOUNT_PATH}{sql_filename}"
    )
...
def app_export_data(args: argparse.Namespace) -> IO[None]:
    db_filename = get_and_verify_db_filename(
        args, "db_filename", must_exist=True
    ) | pipe.Of[Result[str, str]].to(
        lambda res: unwrap_or_print_and_exit_on_err(res).extract(caller_returns_IO=True)
    )
...
def app_sync(args: argparse.Namespace) -> IO[None]:
    ...
    new_table_columns, new_inserts = (
        rename_commands
        | pipe.OfIter[SyncCommandsImpl.Rename].tap(
            lambda command: print("command", command)
        )
        | pipe.OfIter[SyncCommandsImpl.Rename].fold(
            (table_columns, inserts),
            lambda state, command: command.process_rename(*state),
        )
    )
...
def parse_cmdline_args() -> argparse.Namespace:
    ...
    sp = subparsers.add_parser(
        "gen", help="Generates a new db given a dbml schema file"
    )
    sp.add_argument(
        "dbml_schema_filename",
    ...
    sp = subparsers.add_parser("export_data", help="Exports data added to a sqlite3 db")
    sp.add_argument("db_filename", help="The *.db file to use to check for data in")
    sp.add_argument(
        "-d",
        "--datadir",
        type=str,
    ...
    sp = subparsers.add_parser("rust", help="For rust related database generation")
...
def _main() -> IO[None]:
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "rust":
        if len(sys.argv) > 2:
            os.system("dbmint-rs {}".format(" ".join(sys.argv[2:])))
        else:
            os.system("dbmint-rs")
