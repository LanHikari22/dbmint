...
class IO(Generic[IO_T]):
    ...
    def reflect_extract(self) -> IO_T:
        ...
        caller_annotations = caller_frame.frame.f_globals.get(
            caller_function
        ).__annotations__
        ...
        if return_type is not IO:
            raise Exception(
                f"Caller function '{caller_function}' must return an IO object. Expected: {IO}, Got: {return_type}."
            )
        else:
            return self._result
...
class AppError(Generic[AppError_Enums]):
    def __init__(
        self, errno: AppError_Enums, details: str = "", data: Dict[str, Any] = {}
    ):
        self.errno = errno
...
def unwrap_or_print_and_exit_on_err(
    result: Result[Common1_T, str], exit_code: int = 1
) -> IO[Common1_T]:
    if result.is_ok():
